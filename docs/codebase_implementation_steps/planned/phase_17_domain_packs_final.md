...
| **Checklist sections** | §16, §17 (pass 2), §18 (full audit), §19 (success criteria) |
| **ADR refs** | §5.0/§7.0 (MissionBrief/elicit), §7.15 (domain packs), §7.16 (skills), §13.3, §14, §18 (closing rule) |

## Goal

Close out: the domain-pack registration mechanism, skills aligned to the real API, and a deliberate full-spec audit proving facktry matches its ADR.
...
## Part B — Skills pass 2 (`skills/`)

- Re-read every `skills/*/SKILL.md`, including `skills/elicit-mission/SKILL.md`. Update all call examples to the **real** `agent_api` signatures, result envelope shape, and error taxonomy as implemented. Remove any API names that don't exist; add any that do and are missing.
- Skills remain documentation, not runtime: no skill may instruct bypassing govern/admit/smoke/measure. Add a line to `skills/README.md` stating the canonical `agent_api` module path.

## Part C — Conformance sweep (the ADR §14 checklist, verified)
...
8. Ancestors hash-unchanged after corrective trains — cite phase 11 test.
9. **§13.3 / checklist §18: run the entire mandatory test list and check every row green in one pytest run.** Any missing → implement now, do not waive silently.
...
Also sweep ADR §12 (out of scope): confirm none of the excluded subsystems crept into core.
...
- §16 all `[x]`; §17 pass-2 rows `[x]`; **every §18 row `[x]`**; §19 rows `[x]` with one-line evidence citations; all Progress summary rows `[x]`; "Last global review" date bumped.
