...
def test_domain_pack_register_get_and_protocol_surface():
    ...
    assert get_domain("fixture") is pack
    assert get_domain("fixture").required_brief_sections == ["success_case"]
...
def test_run_stage_dispatches_to_registered_pack_inside_governed_run(tmp_path, monkeypatch):
    ...
    register_domain(FixtureDomain())
    result = agent_api.AgentAPI(store).run_stage("fixture_stage", "objective-valid", {"value": "ran"}, domain="fixture")
    assert result.ok
...
def test_unregistered_stage_does_not_improvise(tmp_path, monkeypatch):
    ...
    assert "stage" in result.error["reason"].lower()
